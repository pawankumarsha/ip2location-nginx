...
static ngx_int_t ngx_http_ip2location_get_str_value_comulative(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
...
ngx_http_ip2location_vars[] = {
	{
		ngx_string("ip2location_comulative"), NULL,
		ngx_http_ip2location_get_str_value_comulative,
...
static ngx_http_variable_t 
ngx_http_ip2location_comulative_vars[] = {
	{
...
static ngx_int_t
ngx_http_ip2location_get_str_value_comulative(ngx_http_request_t *r, ngx_http_variable_value_t *val, uintptr_t data)
{
	ngx_http_variable_t     *v;
	ngx_int_t length = 0;
	unsigned char str[516] = {};
	ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "pawan ipaddress (%V)", &r->connection->addr_text);
        for (v = ngx_http_ip2location_comulative_vars; v->name.len; v++) {
		uintptr_t data = v->data;
		ngx_int_t out = ngx_http_ip2location_get_str_value(r, val, data);
		char search_for_string[] = "unavailable";
    		if (!strstr((char *)val->data, search_for_string)) {
			if(val->valid == 1 && out == NGX_OK) {
				int i  = 0;
				while(i < val->len) {
					str[length + i] = val->data[i];
					i++;
				}
				str[length + i] = ',';
				str[length + i + 1] = ' ';
	...
	val->data = str;
	val->len = length;
	ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "pawan final string (%s)", val->data);
	produce_events(r->connection->addr_text.data, str);
        return NGX_OK;	
...
ngx_http_ip2location_get_str_value(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data)
{
	...
	//ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "pawan val is (%s)", val);
	len = ngx_strlen(val);
	...
	ngx_memcpy(v->data, val, len+1);
	//ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "pawan length is: %i, v->data is (%s)",len, v->data);
...
ngx_http_ip2location_get_records(ngx_http_request_t *r)
{
	...
	size = ngx_sock_ntop(addr.sockaddr, p, NGX_INET6_ADDRSTRLEN, 0);
		...
		ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "Pawan %i IP address detected by IP2Location: %s", counter,  p);
		counter ++;
