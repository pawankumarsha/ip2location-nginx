...
int produce_events (unsigned char *ip, unsigned char *data) {
    rd_kafka_t *producer;
    ...
    size_t key_len = strlen((char *)ip);
    size_t value_len = strlen((char *)data);
    ...
    err = rd_kafka_producev(producer,
			...
			RD_KAFKA_V_KEY((void*)ip, key_len),
			RD_KAFKA_V_VALUE((void*)data, value_len),
    ...
    if (err) {
        ...
        g_message("Produced event to topic %s: key = %12s value = %12s", topic, ip, data);
    }
